...
The demo wizard and scenario cards are backed by JSON API endpoints. You can call them with `curl` or any HTTP client. All endpoints require `PG_RETEST_DEMO=true` to be set (configured automatically in Docker); without it, they return HTTP 404.

**Wizard endpoints:**

```bash
# Check demo config
curl http://localhost:8080/api/v1/demo/config

# Run wizard step 1 (explore workload)
curl -X POST http://localhost:8080/api/v1/demo/wizard/1

# Check wizard step status
curl http://localhost:8080/api/v1/demo/wizard/1

# Run steps 2-5 sequentially (each builds on the previous)
curl -X POST http://localhost:8080/api/v1/demo/wizard/2
curl -X POST http://localhost:8080/api/v1/demo/wizard/3   # requires step 2 completed
curl -X POST http://localhost:8080/api/v1/demo/wizard/4
curl -X POST http://localhost:8080/api/v1/demo/wizard/5
```

**Scenario endpoints:**

```bash
# Run a scenario (migration, capacity, ab, tuning)
curl -X POST http://localhost:8080/api/v1/demo/scenario/migration
curl -X POST http://localhost:8080/api/v1/demo/scenario/capacity
curl -X POST http://localhost:8080/api/v1/demo/scenario/ab
curl -X POST http://localhost:8080/api/v1/demo/scenario/tuning

# Check scenario status
curl http://localhost:8080/api/v1/demo/scenario/migration

# Reset Database B
curl -X POST http://localhost:8080/api/v1/demo/reset-db
```

> **Note:** Wizard step 3 (compare) depends on step 2 (replay) having completed first -- step 2 saves replay results to disk that step 3 reads. Running step 3 without step 2 returns HTTP 500. All other steps and scenarios are independent.
...
**Port conflicts:**
The demo uses ports 5450, 5451, and 8080. If these are in use, override them with environment variables:
